...
{  
	...
	while(y0<=y1)						
	{  
		tg_draw_HLine(x0, y0, x1, color);	// 当前画水平线
		y0++;							// 下一行
	}
}
/****************************************************************************
...
{  
	...
	{  
		...
		tg_draw_point(x0, y0, color);	// 显示最后一点
	}
...
void tg_draw_picture(uint8_t x,uint8_t y,const uint8_t *p,uint8_t p_w,uint8_t p_h)
{
	...
	uint8_t y0=y;
	uint8_t width=p_w;
	if(x+p_w>SCR_WIDTH)width=SCR_WIDTH-p_w;//实际显示宽度
	uint8_t high=p_h;
	if(y+p_h>SCR_HIGH)high=SCR_HIGH-p_h;//实际显示高度
...
{  
	...
	if(draw_y0<SCR_HIGH) tg_draw_point(draw_x0, draw_y0, color);	// 90度

	draw_x2 = draw_x3 = x0;
	draw_y2 = draw_y3 = y0 - r;
	if(draw_y2>=0) tg_draw_point(draw_x2, draw_y2, color);			// 270度


	draw_x4 = draw_x6 = x0 + r;
	draw_y4 = draw_y6 = y0;
	if(draw_x4<SCR_WIDTH) tg_draw_point(draw_x4, draw_y4, color);	// 0度

	draw_x5 = draw_x7 = x0 - r;
	draw_y5 = draw_y7 = y0;
	if(draw_x5>=0) tg_draw_point(draw_x5, draw_y5, color);			// 180度   
...
{  
	...
	draw_y0 = draw_y1 = y0 + r;
	if(draw_y0<SCR_HIGH)
		tg_draw_point(draw_x0, draw_y0, color);	// 90度

	draw_x2 = draw_x3 = x0;
	draw_y2 = draw_y3 = y0 - r;
	if(draw_y2>=0)
		tg_draw_point(draw_x2, draw_y2, color);	// 270度

	draw_x4 = draw_x6 = x0 + r;
	draw_y4 = draw_y6 = y0;
	if(draw_x4<SCR_WIDTH) 
	{  
		tg_draw_point(draw_x4, draw_y4, color);	// 0度
		fill_x1 = draw_x4;
	}
	else
		fill_x1 = SCR_WIDTH;
	...
	draw_y5 = draw_y7 = y0;
	if(draw_x5>=0) 
		tg_draw_point(draw_x5, draw_y5, color);	// 180度
	if(1==r) return;
...
{  
	...
	tg_draw_point(draw_x0, draw_y0, color);					// 画y轴上的两个端点 
	tg_draw_point(draw_x2, draw_y2, color);
...
/****************************************************************************
* 名称：tg_read_rightPoint()
* 功能：找出指定点右边最近的非color点。
* 入口参数： x0		指定点的x轴坐标值
*           y0		指定点的y轴坐标值
*           color	指定颜色值
* 出口参数：返回该点的x轴坐标值。
* 说明：若没有找出，则返回最右的x坐标SCR_WIDTH。
****************************************************************************/
...
{  
   ...
   tg_read_point(x0, y0, &fcolor);						// 取得填充点的颜色
   if( GUI_CmpColor(fcolor,color)!=0 ) return;
   
   y0_bak = y0;
   x0_bak = xx0 = tg_read_leftPoint(x0, y0, fcolor);		// 找出当前y坐标上的最左边的点
   x1_bak = xx1 = tg_read_rightPoint(x0, y0, fcolor);		// 找出当前y坐标上的最右边的点
...
FILL_UP:  
   ...
   {  yy += 1;											// 中心点向上一点
      ...
      {  y0 = xx0;
         x0 = y0-1;
         for(i=y0; i<=xx1; i++)
         {  if( tg_cmp_pointColor(i, yy-1, fcolor)==0 )	// 更新折点
            {  y0 = i;										
            }
            else
            {  if(x0!=y0)								// 找到新的折点
      ...
      xx = (xx1 + xx0)/2;								// 更新中心点
      tg_draw_HLine(xx0, yy, xx1, color);					// 填充一行
...
{  int32_t  draw_x, draw_y;
   ...
   {  case  1:
            ...
            while(1)
            {  tg_draw_point(draw_x, draw_y, color);		// 开始画图
...
{  int32_t  draw_x, draw_y;					// 画图坐标变量
   ...
   {  /* 计算下一点 */
      ...
      if(draw_on==1) tg_draw_point(draw_x, draw_y, color);			// 开始画图
      pno_angle++;
